...
func run() -> Array[String]:
	...
	var food_position := prey.global_position + Vector2(170.0, 0.0)
	var resource := INTEGRATION.spawn_resource(world, "grass_patch", food_position)
	TEST_UTILS.expect(resource != null, failures, "The test should spawn a control grass resource")
	if resource == null:
		await INTEGRATION.shutdown_main(context)
		return failures

	prey.global_position = food_position - Vector2(170.0, 0.0)
	...
	var distance_after := prey.global_position.distance_to(resource.global_position)

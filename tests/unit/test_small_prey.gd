...
func _test_small_prey_builds_decision_context(failures: Array[String]) -> void:
	...
	TEST_UTILS.expect_equal(str(context_data.get("hunger_stage", "")), "hungry", failures, "Small prey decision context should include hunger stage")
	TEST_UTILS.expect_equal(str(context_data.get("current_behavior", "")), "wander", failures, "Small prey decision context should expose behavior")
	prey.queue_free()

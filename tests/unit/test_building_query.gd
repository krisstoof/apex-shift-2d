...
const TEST_UTILS := preload("res://tests/test_utils.gd")
...
func test_get_buildings_near_filters_by_distance() -> void:
	...
	expect(result.has(nearby_wall), "Should include nearby wall")
...
func test_get_buildings_near_uses_distance_squared_optimization() -> void:
	...
	expect(result.has(at_radius), "Should include building at boundary")
...
func test_get_buildings_near_ignores_invalid_nodes() -> void:
	...
	expect(result.has(valid_wall), "Should include valid wall")
...
func test_get_buildings_near_with_multiple_types_and_distances() -> void:
	...
	expect(nearby_walls.has(close_wall), "Should include close_wall")
	...
	expect(nearby_traps.has(close_trap), "Should include close_trap")
...
func test_get_buildings_near_returns_array_copy() -> void:
	...
	expect(result1 != result2 or result1 == result2, "Arrays can be same object or different")
...
func test_get_buildings_near_with_zero_radius() -> void:
	...
	expect(result.has(at_position), "Should include building at position")

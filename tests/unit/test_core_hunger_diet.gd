...
const HUNGER_DIET := preload("res://scripts/core/survival/hunger_diet.gd")
...
func _test_desperate_search_radius_switches(failures: Array[String]) -> void:
	...
	TEST_UTILS.expect_close(diet.get_food_search_radius(), float(GAME_BALANCE.ANIMAL_AI.get("food_search_radius", 520.0)), failures, "Non-desperate creatures should use the normal food search radius")
	diet.hunger = 0.90
	TEST_UTILS.expect_close(diet.get_food_search_radius(), float(GAME_BALANCE.ANIMAL_AI.get("desperate_food_search_radius", 780.0)), failures, "Desperate creatures should use the wider food search radius")

func _make_diet(hunger_value: float, max_hunger_value: float, hunger_growth_rate_value: float, energy_value: float) -> HUNGER_DIET:
	var diet := HUNGER_DIET.new()
	diet.configure({
		"hunger": hunger_value,

...
func _test_hud_inventory_screen_lists_inventory_items(failures: Array[String]) -> void:
	...
	if slot_nodes.size() >= 9:
		...
		TEST_UTILS.expect_equal((slot3.get_node("CountLabel") as Label).text, "x6", failures, "Stone x6 should show its full stack amount")
		TEST_UTILS.expect_equal((slot4.get_node("CountLabel") as Label).text, "x12", failures, "Fiber x12 should show its full stack amount")
		TEST_UTILS.expect_equal((slot5.get_node("CountLabel") as Label).text, "x2", failures, "Meat x2 should show its full stack amount")
		TEST_UTILS.expect_equal((slot6.get_node("CountLabel") as Label).text, "x3", failures, "Bone x3 should show its full stack amount")
		TEST_UTILS.expect_equal((slot7.get_node("CountLabel") as Label).text, "x1", failures, "Torch x1 should show its full stack amount")

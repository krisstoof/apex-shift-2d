...
const BACKGROUND_HUNGER_RATE := 0.018
const BACKGROUND_ENERGY_RECOVERY_RATE := 0.015
...
func _apply_background_life_tick(elapsed_seconds: float) -> void:
	...
	hunger = clamp(hunger + BACKGROUND_HUNGER_RATE * elapsed_seconds, 0.0, 1.0)
	energy = clamp(energy + BACKGROUND_ENERGY_RECOVERY_RATE * elapsed_seconds, 0.0, 1.0)
	attack_cooldown = max(attack_cooldown - elapsed_seconds, 0.0)
	target_lock_time = max(target_lock_time - elapsed_seconds, 0.0)

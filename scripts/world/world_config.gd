...
const RESOURCE_DENSITY_MULTIPLIER := 2.05
const DECORATIVE_VEGETATION_DENSITY_MULTIPLIER := 2.85
const ROCK_DENSITY_MULTIPLIER := 1.95
const FOOD_BUSH_DENSITY_MULTIPLIER := 2.05
...
static func generate_landmarks(world_seed: int) -> Array[Dictionary]:
	# Ponds and hills are now generated by topography, not landmark runtime data.
	# Keep this for future special POIs.
	return []

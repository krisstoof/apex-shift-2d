...
func decide(_state, context) -> CreatureDecision:
	...
	if ctx.has_building_hazard:
		return DECISION.for_action(DECISION.ACTION_AVOID_BUILDING, "avoid_building_%s" % ctx.building_hazard_kind, ctx.building_hazard_position, ctx.building_hazard_entity_id, ctx.building_hazard_kind)
	...
	if ctx.is_hungry() and ctx.has_food_target and ctx.food_kind != "meat" and ctx.food_in_eat_range and ctx.eat_cooldown <= 0.0:
		return DECISION.for_action(DECISION.ACTION_EAT, "hungry_plant_in_range", ctx.food_position, ctx.food_entity_id, ctx.food_kind, {"consume": true})
	if ctx.is_hungry() and ctx.has_food_target and ctx.food_kind != "meat":
		return DECISION.for_action(DECISION.ACTION_SEEK_FOOD, "hungry_seek_plants", ctx.food_position, ctx.food_entity_id, ctx.food_kind)
	if ctx.is_hungry() and ctx.has_prey_target:
		return DECISION.for_action(DECISION.ACTION_HUNT, "hungry_no_plants_hunt", ctx.prey_position, ctx.prey_entity_id, ctx.prey_kind, {"attack": ctx.prey_in_attack_range})
	if ctx.is_hungry() and ctx.has_meat_target:
		return DECISION.for_action(DECISION.ACTION_SEEK_FOOD, "hungry_scavenge", ctx.meat_position, ctx.meat_entity_id, "meat")
	if ctx.should_rest or ctx.energy <= 0.14:
